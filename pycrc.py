"""
    This python library is a cheaply made, barely tested conversion from the c library
    libcrc-2.0 which was stolen straight from https://www.libcrc.org/

    The table was generated as part of the aforementioned library, but it's hard code it since it's
    not ever going to change.

    and confirmed with https://crccalc.com/  using the CRC-16/MODBUS algorithm
"""
table = [0x0000, 0xc0c1, 0xc181, 0x0140, 0xc301, 0x03c0, 0x0280, 0xc241, 0xc601, 0x06c0, 0x0780,
...
def crc16_modbus(ascii_String):
    crcresult = 0xFFFF
    for i in range(0, len(ascii_String)):
        crcresult = update_crc(crcresult, ascii_String[i])
    return crcresult
    
def main():
    print("Test of crc")
    test = b"$TEA"
    crcresult = crc16_modbus(test)
    print(hex(crcresult))
